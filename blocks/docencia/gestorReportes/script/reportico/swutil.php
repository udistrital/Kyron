...
$in_driver, $in_dbname, $in_hostname, $in_sql, $in_conn = false
) {
    ...
    if (!$in_conn) {
        ...
        $rs = $ado_connection->Execute($in_sql) or die("Query failed : " . $ado_connection->ErrorMsg());
    } else {
        $rs = $in_conn->Execute($in_sql) or die("Query failed : " . $in_conn->ErrorMsg());
    }
...
function get_output_encoding_html() {
    ...
    switch (SW_OUTPUT_ENCODING) {
        case "None" : $txt = '';
            break;
        case "UTF8" : $txt = $tmp1 . "utf-8" . $tmp2;
            '<meta charset="utf-8">';
            break;
        default : $txt = $tmp1 . SW_OUTPUT_ENCODING . $tmp2;
            break;
    }
