...
if(!isset($_REQUEST["opcion"])|| (isset($_REQUEST["opcion"])&&$_REQUEST["opcion"]=="nuevo")){
	...
	$cadenaConfirmar="data: datos,
			loadComplete: function () {
					
					var valorSubtotal=jQuery('".$nombreTabla."').getCol('subtotal', false, 'sum');
					var valorIva=jQuery('".$nombreTabla."').getCol('totalIva', false, 'sum');
					var valorTotal=jQuery('".$nombreTabla."').getCol('total', false, 'sum');
					
					$('".$nombreTabla."').jqGrid('footerData','set', {marca: 'Totales:', 
...
<script type='text/javascript'>
	...
	$(function() {
		...
		?>],
		    ...
		    viewrecords: true,
		    sortorder: "desc",
		    //@todo: Es necesario sincronizar el ancho conforme al estilo del usuario
	        width: 750,
	        footerrow: true,
	        userDataOnFooter: true,
	        <?php if(isset($cadenaConfirmar)){
	        	echo $cadenaConfirmar;
	        }
	        	?>
	        datatype: 'local',	
	        editurl: 'clientArray',
