	...
	getColumnIndex = function (columnName) {
	 ...
	 onclickSubmitLocal = function (options, postdata) {
        ...
        if (grid_p.treeGrid === true) {
            ...
            var filaSubtotal=0;		//Precio * Cantidad
            var filaDescuento=0;	
            var filaIva=0;			//filaSubtotal*Iva/100
            var filaTotal=0; 		//filaSubtotal+filaIva-filaDescuento;
			...
			for(a=0;a<filas.length;a++)
			 {
			    ...
			    filaSubtotal= parseFloat(row.precio)*parseFloat(row.cantidad);
			    
			    filaDescuento= parseFloat(row.descuento);

			    filaIva=filaSubtotal*parseFloat(row.iva)/100;

			    filaTotal=filaSubtotal+filaIva-filaDescuento;

			    //Colocar los valores en los campos no editables que corresponde 
			    jQuery(miGrilla).setCell(filas[a],"subtotal", filaSubtotal.toString());
			    jQuery(miGrilla).setCell(filas[a],"totalIva", filaIva.toString());
			    jQuery(miGrilla).setCell(filas[a],"total", filaTotal.toString());

			    //Totales de la entrada 

			    valorSubtotal=valorSubtotal+ filaSubtotal;
			    valorDescuento=valorDescuento+filaDescuento;
			    valorIva=valorIva+ filaIva;
			    valorTotal=valorTotal+filaTotal;
			
			 };

			//Colocar los valores totales de la entrada 
			datosTotales ={ marca: "Totales:", 
							 subtotal: valorSubtotal,
							 totalIva: valorIva,
							 total: valorTotal
							};				
			jQuery(miGrilla).footerData("set",datosTotales);			
        }
